...
# After EXCLUDE_DIRS definition, add:
IMAGE_EXTENSIONS = {
    ".svg",
    ".png",
    ".jpg",
...
def parse_ignore_file(ignore_file_path: str) -> Tuple[Set[str], Set[str]]:
    ...
    for line in lines:
        ...
        is_directory_pattern = (
            ...
            or clean_pattern
            in ["node_modules", "dist", "build", "__pycache__", ".git", "venv", ".venv"]
        )
    ...
    if negation_patterns_found:
        print(
            f"   ⚠️  Note: {len(negation_patterns_found)} negation patterns (!pattern) found but not supported yet"
        )

    return dirs_to_exclude, files_to_exclude


def update_exclusions_from_ignore_files(
    ...
    if additional_exclude_dirs:
        print(
            f"   📁 Directory patterns: {', '.join(sorted(list(additional_exclude_dirs)[:10]))}"
        )
    ...
    if additional_exclude_files:
        print(
            f"   📄 File patterns: {', '.join(sorted(list(additional_exclude_files)[:10]))}"
        )
...
class FileAnalyzer:
    ...
    def analyze_file(self, filepath: str) -> Dict:
        ...
        # In analyze_file method, after checking binary files:
        return {
            "lines": lines,
...
"""
    ...
    # After the existing LLM_CONTEXT_DIRECTIVE section, add:
    instructions_text = "\n".join(
        [f"{i+1}. {inst}" for i, inst in enumerate(LLM_CONTEXT_INSTRUCTIONS)]
    )
    # Insert instructions_text into the llm_instructions string
    llm_instructions = instructions_text + "\n" + llm_instructions
...
def gather_files_enhanced(
    ...
    if respect_ignore_files:
        additional_exclude_dirs, additional_exclude_files = (
            update_exclusions_from_ignore_files(paths)
        )
        ...
        if os.path.isfile(p):
            ...
            for root, dirs, files in os.walk(p):
                ...
                for fname in files:
                    ...
                    if not is_excluded_enhanced(
                        fp
                    ) and not should_exclude_file_enhanced(fp):
                        if not extensions or any(
                            fp.endswith(ext) for ext in extensions
                        ):
                            ...
                            if abs_path in priority_set or file_size <= max_file_size:
                                ...
                                print(
                                    f"⚠️  Skipping large file: {fp} ({file_size} bytes)"
                                )
...
def write_file_content(
    ...
    is_binary = metadata.get("error") == "Binary file"

    if is_image or is_binary:
        out_file.write(f"<<< FILE: {rel_path} >>>\n")
        placeholder = "Static asset (image)" if is_image else "Binary file"
        out_file.write(f"# {placeholder}: Content omitted.\n")
        out_file.write("<<< END FILE >>>\n\n")
        return 4, False  # Return lines written and truncation status
    ...
    out_file.write(f"<<< FILE: {rel_path} >>>\n")
    out_file.write(
        f"# Metadata: type={file_type}, lines={metadata.get('lines', 0)}, "
    ...
    try:
        content, was_truncated = truncate_large_content_streaming(
            filepath, max_lines_per_file
        )
...
def write_table_of_contents(file_data: List[Tuple[str, Dict]], out_file) -> int:
    ...
    for i, (filepath, metadata) in enumerate(file_data, 1):
        ...
        out_file.write(
            f"{i:3d} │ {rel_path[:56].ljust(56)} │ {file_type[:10].ljust(10)} │ {lines:5d} │ {tokens:6d} │ {priority:7d}\n"
        )
        lines_written += 1
...
def write_enhanced_output(
    ...
    with open(output_path, "w", encoding="utf-8") as out:
        ...
        out.write(
            f"# Summary: {total_files} files processed, {total_truncated} truncated\n"
        )
        total_lines_written += 2
...
Examples:
  ...
  python merge_for_llm.py -f . --remove-spaces aggressive --no-toc
    ...
    parser.add_argument(
        "-f", "--folders", nargs="+", required=True, help="Folders and files to process"
    )
    parser.add_argument(
        "--ext",
        "--extensions",
    ...
    parser.add_argument(
        "--no-toc", action="store_true", help="Skip table of contents to save space"
    )
    parser.add_argument(
        "--no-ignore",
        action="store_true",
